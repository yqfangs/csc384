...
if __name__ == '__main__':
    ...
    # prob1 = VE(car, bv, [cs])
    # prob2 = VE(car, bv, [al, cs])
    # print("P(bv|cs)")
    # for i in range(len(prob1)):
    #     print("P({0:} = {1:}) = {2:0.1f}".format(bv.name, bv.domain()[i], 100 * prob1[i]))
    # print()
    # print("P(bv|al,cs)")
    # for i in range(len(prob2)):
    #     print("P({0:} = {1:}) = {2:0.1f}".format(bv.name, bv.domain()[i], 100 * prob2[i]))
    # print()
    #
    # #
    # charging_sys_w_battery_age_no_voltage = VE(car, cs, [ba])
    # charging_sys_wo_battery_age_no_voltage = VE(car, cs, [])
    # print("P(cs|ba)")
    # for i in range(len(charging_sys_w_battery_age_no_voltage)):
    #     print("P({0:} = {1:}) = {2:0.1f}".format(cs.name, cs.domain()[i], 100 * charging_sys_w_battery_age_no_voltage[i]))
    # print()
    # #
    # print("P(cs)")
    # for i in range(len(charging_sys_wo_battery_age_no_voltage)):
    #     print("P({0:} = {1:}) = {2:0.1f}".format(cs.name, cs.domain()[i], 100 * charging_sys_wo_battery_age_no_voltage[i]))
    # print()
    # #
    # charging_sys_w_battery_age_w_voltage = VE(car, cs, [bv, ba])
    # charging_sys_wo_battery_age_w_voltage = VE(car, cs, [bv])
    # print("P(cs|bv,ba)")
    # for i in range(len(charging_sys_w_battery_age_w_voltage)):
    #     print("P({0:} = {1:}) = {2:0.1f}".format(cs.name, cs.domain()[i], 100 * charging_sys_w_battery_age_w_voltage[i]))
    # print()
    # print("P(cs|bv)")
    # for i in range(len(charging_sys_w_battery_age_w_voltage)):
    #     print(
    #         "P({0:} = {1:}) = {2:0.1f}".format(cs.name, cs.domain()[i], 100 * charging_sys_wo_battery_age_w_voltage[i]))
    # print()
    #
    # l = [asys, fs, tm, sq, cc]
    # evidences = []
    # for var in l:
    #     var.set_evidence(var.domain()[0])
    #     evidences.append(var)
    #     print("Evidences: {} = {}".format(var.name, var.get_evidence()))
    #     prob = VE(car, st, evidences)
    #     print(
    #             "P({0:} = {1:}) = {2:0.1f}".format(st.name, st.domain()[0], 100 * prob[0]))

    l = [ sq, asys, fs, tm, cc]
    evidences = []
    for var in l:
        var.set_evidence(var.domain()[1])
        evidences.append(var)
        print("Evidences: {} = {}".format(var.name, var.get_evidence()))
        prob = VE(car, st, evidences)
        print(
                "P({} = {}) = {}".format(st.name, st.domain()[0], prob[0]))
