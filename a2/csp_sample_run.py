...
def nQueens(n):
    ...
    for qi in range(len(dom)):
        for qj in range(qi+1, len(dom)): # 这样的forloop保证所有ij搭配都covered
            con = Constraint("C(Q{},Q{})".format(qi+1,qj+1),[vars[qi], vars[qj]]) # vars里都是Variables,
            # format里+1是因为range从0开始，但var name从1开始
            sat_tuples = []
            for t in itertools.product(dom, dom): # 1个t是一个tuple, tuples是所有 dom pair combination
